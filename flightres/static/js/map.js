...
$(document).ready(function () {
    ...
    function createModal(data, das, lat, long,alti, status1,id) {
        ...
        let approve_button = (current_email===data.assigned_to__email && data.status == 'Rejected')?`<a href="/np/dashboard/approve_perm/`+ data.uav_uid + `/` + data.assigned_to__username + `/approve" class="common-button is-bg">Approve</a>`:`<a class="common-button is-bg is-disable">Approve</a>`
        let deny_button = (current_email===data.assigned_to__email && data.status == 'Approved')?`<span  class="common-button is-border cancel-button" id="denyButton">Deny</span>`:`<span  class="common-button is-border cancel-button is-disable">Deny</span>`
